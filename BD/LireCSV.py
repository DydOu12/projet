...
nameAct = "../ressources_csv/Activite.csv"
fileAct = open(nameAct, "r")

nameInst = "../ressources_csv/Installations.csv"
fileInst = open(nameInst, "r")

nameEquip = "../ressources_csv/Equipements.csv"
...
tableAdmin = input("Créer une table administrateur ? (Y/N) ")

if(tableAdmin == "Y"):
	CreateTableAdmin()
	premierNom = input("Entrer le nom de l'administrateur :")
	premierpassword = input("Entrer le mot de passe pour " + premierNom + " : ")
	
	#Cryptage ddu mot de passe de l'admin grâce au SHA-1


	hash_object = hashlib.sha1(premierpassword.encode())
	hex_digPassword = hash_object.hexdigest()
	
	AddEntryAdmin(premierNom,hex_digPassword)
	
	print("L'administrateur a bien été créé.")
	
	creationAutreAdmin = input("Créer un autre administrateur ? (Y/N)")
	
	while(creationAutreAdmin == "Y"):
		autreNom = input("Entrer le nom de l'administrateur :")
		autrePassword = input("Entrer le mot de passe pour " + autreNom + " : ")
		
		#Cryptage du mot de passe de l'admin grâce au SHA-1
		
		hash_object = hashlib.sha1(autrePassword.encode())
		hex_digPassword = hash_object.hexdigest()
		
		AddEntryAdmin(autreNom,hex_digPassword)
		print("L'administrateur a bien été créé.")
		creationAutreAdmin = input("Créer un autre administrateur ? (Y/N)")
...
#test = conn.execute('SELECT * FROM Admin')
